...
void Renderer::init() {
    ...
    glLoadIdentity();
    glTranslatef(-this->ui->camera.position.x, -this->ui->camera.position.y, -this->ui->camera.position.z);
    glRotatef(radToDeg(-this->ui->camera.angle), this->ui->camera.axis.x, this->ui->camera.axis.y, this->ui->camera.axis.z);
...
void Renderer::drawPrimitive(Primitive *prm) {
    assert(this->scene->prm_tessellation_start.find(prm) != this->scene->prm_tessellation_start.end());
    ...
    float ambientColor[3] = {color.r * ambient, color.g * ambient, color.b * ambient};
    float diffuseColor[3] = {color.r * diffuse, color.g * diffuse, color.b * diffuse};
    float specularColor[3] = {color.r * specular, color.g * specular, color.b * specular};
...
void Renderer::display() {
    ...
    // newly added, aim to retrieve the only primitive in the scene world
    // Created by MA LEIYA 2016-5-28
    Renderable* ren = NULL;
    //char sphere;
    ...
    // this is the place where to tell OPENGL to update renderers
    // scene->update();
    // glutPostRedisplay();
